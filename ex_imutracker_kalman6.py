...
def calibrate(gyro, accelerometer):
    ...
    while calibCount < 100:
        ...
        data_ms = 100 # 100ms
        acc = accelerometer.read_imu(data_ms)  # data cover 100ms
        aRes = numpy.array(acc)
    ...
    aMV = [cGyVar, cGyMean, cAcVar, cAcMean, cAnglesVar, cAngleMean]
    return aMV


def kalmanFilter(gyro, accelerometer, aMV, debug=0):
    global x
    global out
    rotationCorrection = [0, 0, 0]
    ...
    while True:
        ...
        acc_read = accelerometer.read_imu(data_ms)
        gyr_read = gyro.read_imu(data_ms)
        if acc_read is None or gyr_read is None:
            break
    
        aRes = numpy.array(acc_read)
        grys = gyr_read - cGyMean  # read gyro and remove calibration error
        ...
        poss.append(x[3:6])
        if debug == 1:
            print('Angle: ', x[0:3], 'Velocity:', x[6:9], 'Position:', x[3:6])
    return poss
...
class GyroReader:
    def __init__(self, data_imu):
        self.data_gyo = data_imu[0:, 3:]
        self.cur_ndx = 0

    def read_imu(self, read_ms):
...
def track_by_kalman6(data_imu, mode=None):
    ...
    aMV = calibrate(gyro, accelerometer)
    pos = kalmanFilter(gyro, accelerometer, aMV, debug=0)
    return np.array(pos)
