...
class IMUTracker_Fuse6:
    ...
    def initialize(self, data, noise_coefficient=None):

        noise_coefficient = {'w': 50, 'a': 50, 'm': 10}

        if noise_coefficient is None:
            noise_coefficient = {'w': 100, 'a': 100, 'm': 10}
        ...
        print('acc noise: %s, bias: %s' % (gyro_noise, gyro_bias))
        print('ang noise: %s, bias: %s' % (acc_noise, acc_bias))
        #mag_noise = noise_coefficient['m'] * np.linalg.norm(mvar)
        #return (gn, g0, mn, gyro_noise, gyro_bias, acc_noise, mag_noise)
        return (gn, g0, gyro_noise, gyro_bias, acc_noise, acc_bias)
    ...
    def attitudeTrack(self, data, init_list):
        ...
        # ---- Initialization ----
        # ------------------------------- #
        #gn, g0, _, gyro_noise, gyro_bias, acc_noise, _ = init_list
        gn, g0, gyro_noise, gyro_bias, acc_noise, acc_bias = init_list
        w = data[:, self._widx[0]:self._widx[1]] - gyro_bias
        #a = data[:, self._aidx[0]:self._aidx[1]] 
        a = data[:, self._aidx[0]:self._aidx[1]] - acc_bias
        #m = data[:, self._midx[0]:self._midx[1]]
        ...
        while t < sample_number:
            ...
            Eps = np.vstack(normalized(at) - pa)
